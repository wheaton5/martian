...
func runWebServer(uiport string, martianVersion string, sman *SubmissionManager) {
	...
	m.Use(martini.Static(util.RelPath("../web/houston/serve"), martini.StaticOptions{"/", true, "index.html", nil}))
	m.Use(martini.Static(util.RelPath("../web/houston/client"), martini.StaticOptions{"", true, "index.html", nil}))
	...
	app.Get("/metrics", func() string {
		return util.Render("web/houston/templates", "metrics.html",
			&MainPage{
	...
	app.Get("/api/get-submissions", func() string {
		return util.MakeJSON(sman.EnumerateSubmissions())
	})
