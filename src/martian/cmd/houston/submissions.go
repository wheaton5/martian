...
func (self *SubmissionManager) loadCache() error {
	...
	if err != nil {
		util.LogInfo("submngr", "Could not read cache file %s.", self.cachePath)
		return err
	}

	if err := json.Unmarshal(bytes, &self.cache); err != nil {
		util.LogError(err, "submngr", "Could not parse JSON in cache file %s.", self.cachePath)
		return err
	}

	util.LogInfo("submngr", "%d pipestances loaded from cache.", len(self.cache))
	return nil
}
...
func writeJson(fpath string, object interface{}) {
	...
	if err := ioutil.WriteFile(fpath, bytes, 0644); err != nil {
		util.LogError(err, "submngr", "Could not write JSON file %s.", fpath)
	}
}
...
func (self *SubmissionManager) loadSubmission(source, date, name string) *Submission {
	...
	if fi, err := os.Lstat(path.Join(p, "HEAD")); err == nil && (fi.Mode()&os.ModeSymlink == os.ModeSymlink) {
		...
		util.LogInfo("submngr", "Discovered %s %s at %s", state, kind, key)

		util.LogInfo("submngr", "    Immortalizing")
		serial, ok := self.GetPipestanceSerialization(source, date, name, core.FinalState)
		if ok {
		...
		if state == "complete" {
			...
			for _, psp := range self.pipestanceSummaryPaths {
				...
				if _, err := os.Stat(summaryPath); err == nil {
					...
					if err != nil {
						util.LogInfo("submngr", "    Could not read summary file %s.", summaryPath)
						return nil
					}
					if err := json.Unmarshal(bytes, &summary); err != nil {
						util.LogError(err, "submngr", "    Could not parse JSON in file %s.", summaryPath)
						return nil
					}
					util.LogInfo("submngr", "    Summary exists at %s", psp)
					break
				}
			}
		}
		...
		util.LogInfo("submngr", "Discovered %s %s at %s", kind, fname, key)
	}
...
func (self *SubmissionManager) GetPipestance(container string, pname string, psid string, readOnly bool) (*core.Pipestance, bool) {
	...
	if err != nil {
		util.LogError(err, "submngr", "Failed to reattach to %s/%s/%s", container, pname, psid)
		return nil, false
	}
...
func (self *SubmissionManager) GetPipestanceSerialization(container string, pname string, psid string, name core.MetadataFileName) (interface{}, bool) {
	...
	if err := pipestance.Immortalize(true); err != nil {
