...
Options:
	...
	if !readonly {
		util.LogTee(path.Join(env["HOUSTON_LOGS_PATH"],
			time.Now().Format("20060102150405")+".log"))
	} else {
		util.Log("Starting up in readonly mode")
	}
	...
	if err != nil {
		util.LogError(err, "Could not parse HOUSTON_DOWNLOAD_INTERVALMIN value %s",
			env["HOUSTON_DOWNLOAD_INTERVALMIN"])
		os.Exit(1)
	}
	downloadMaxMB, err := strconv.Atoi(env["HOUSTON_DOWNLOAD_MAXMB"])
	if err != nil {
		util.LogError(err, "Could not parse HOUSTON_DOWNLOAD_MAXMB value %s",
