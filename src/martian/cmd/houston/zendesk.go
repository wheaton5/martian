...
func (self *ZendeskDownloadable) Download(dstPath string) {
	...
	if err != nil {
		util.LogError(err, "zendesk", "    Could not create file %s for download", dstPath)
		return
	}
	defer fd.Close()
	...
	if err != nil {
		util.LogError(err, "zendesk", "    HTTP GET failed")
		return
	}
	...
	if err != nil {
		util.LogError(err, "zendesk", "    Download stream copy failed")
		return
	}
	util.LogInfo("zendesk", "    Downloaded %s", humanize.Bytes(uint64(numBytes)))
}
...
func (self *ZendeskDownloadSource) Enumerate() []Downloadable {
	...
	if err != nil {
		util.LogError(err, "zendesk", "Search failed")
		return []Downloadable{}
	}
	results := &zego.Search_Results_Tickets{}
	err = json.Unmarshal([]byte(resource.Raw), results)
	if err != nil {
		util.LogError(err, "zendesk", "Failed to deserialize search results.")
		return []Downloadable{}
	}
	util.LogInfo("zendesk", "Search returned %d tickets", results.Count)
	...
	util.LogInfo("zendesk", "Search returned %d attachments", len(downloadables))
	return downloadables
}
...
func (self *ZendeskDownloadSource) getPipestances(results []*zego.Ticket, auth zego.Auth) []Downloadable {
	...
	for _, t := range results {
		...
		if err != nil {
			util.LogError(err, "zendesk", "Failed to find user %d", t.RequesterId)
			continue
		} else if user == nil || user.User == nil {
			util.LogInfo("zendesk", "Failed to find user %d", t.RequesterId)
			continue
		}
		...
		if err != nil {
			util.LogError(err, "zendesk", "Failed to parse date %s", date)
			continue
		}
		godate = godate.Local()
		y := godate.Year()
		...
		if err != nil {
			util.LogError(err, "zendesk", "ListComments failed for ticket %s", ticket_id)
			continue
		}
