...
func (self *PipestanceManager) loadCache() {
	...
	if err != nil {
		util.LogError(err, "keplerd", "Failed to load pipestance caches: %s", err.Error())
		os.Exit(1)
	}
...
func (self *PipestanceManager) recursePath(root string) []string {
	...
	if _, err := os.Stat(invocationPath); err == nil {
		...
		if _, err := os.Stat(finalStatePath); err == nil {
			util.LogInfo("keplerd", "Found pipestance %s", root)
...
func (self *PipestanceManager) parseFinalState(psPath string) (string, string, map[string]interface{}, error) {
	...
	if len(v) == 0 {
		return "", "", args, &util.MartianError{fmt.Sprintf("Pipestance %s has empty _finalstate file", psPath)}
	}
...
func (self *PipestanceManager) writePerf(psPath string) error {
	...
	if _, err := os.Stat(perfPath); err != nil {
		...
		if state != "complete" {
			return &util.MartianError{fmt.Sprintf("Pipestance %s is not complete", psPath)}
		}
...
func (self *PipestanceManager) InsertPipestance(psPath string) error {
	...
	if self.searchCache(fqname, pipelinesVersion) {
		return &util.MartianError{fmt.Sprintf("Pipestance %s has duplicate fqname %s and version %s", psPath, fqname, pipelinesVersion)}
	}
	...
	if len(nodes) == 0 {
		return &util.MartianError{fmt.Sprintf("Pipestance %s has empty _perf file", psPath)}
	}
...
func (self *PipestanceManager) InsertPipestances(newPsPaths []string) {
	for _, newPsPath := range newPsPaths {
		util.LogInfo("keplerd", "Adding pipestance %s", newPsPath)
		if err := self.InsertPipestance(newPsPath); err != nil {
			util.LogError(err, "keplerd", "Failed to add pipestance %s: %s",
				newPsPath, err.Error())
	...
	for i := 0; i < self.numPerfHandlers; i++ {
		go func() {
			for {
				...
				util.LogInfo("keplerd", "Writing _perf for pipestance %s", psPath)
				if err := self.writePerf(psPath); err != nil {
					util.LogError(err, "keplerd", "Failed to write _perf for pipestance %s: %s",
						psPath, err.Error())
				}
