...
func GetSampleTags(sample *Sample, fastq_paths map[string]string, instanceName string) []string {
	tags := []string{util.MakeTag("instance", instanceName)}

	// Flowcells
	for _, sample_def := range sample.Sample_defs {
		sd_fcid := sample_def.Sequencing_run.Name
		tags = append(tags, util.MakeTag("flowcell", sd_fcid))
		tags = append(tags, util.MakeTag("read1_length", strconv.Itoa(sample_def.Sequencing_run.Read1_length)))
		tags = append(tags, util.MakeTag("read2_length", strconv.Itoa(sample_def.Sequencing_run.Read2_length)))
	}
	...
	fastqFiles, fastqBytes := util.GetDirectorySize(fastq_paths_str)
	fastqFilesTag := util.MakeTag("fastq_files", strconv.Itoa(int(fastqFiles)))
	fastqBytesTag := util.MakeTag("fastq_bytes", strconv.FormatUint(fastqBytes, 10))
...
func GetPreprocessTags(run *Run, fcid string, instanceName string) []string {
	tags := []string{util.MakeTag("instance", instanceName), util.MakeTag("flowcell", fcid)}

	// Number and size of all bcl files
	bclPath := path.Join(run.Path, "Data/Intensities")
	bclFiles, bclBytes := util.GetDirectorySize([]string{bclPath})
	bclFilesTag := util.MakeTag("bcl_files", strconv.Itoa(int(bclFiles)))
	bclBytesTag := util.MakeTag("bcl_bytes", strconv.FormatUint(bclBytes, 10))
...
func runWebServer(uiport string, instanceName string, martianVersion string, rt *core.Runtime,
	...
	// Render: main UI.
	app.Get("/", func() string {
		return util.Render("web/marsoc/templates", "marsoc.html",
			&MainPage{
	...
	app.Get("/pipestances", func() string {
		return util.Render("web/marsoc/templates", "pipestances.html",
			&MainPage{
				InstanceName:     instanceName,
				Admin:            false,
				MarsocVersion:    martianVersion,
	...
	app.Get("/api/get-pipestances", func() string {
		...
		wg.Wait()
		return util.MakeJSON(pipestances)
	})
	...
	app.Get("/metasamples", func() string {
		return util.Render("web/marsoc/templates", "metasamples.html",
			&MainPage{
				InstanceName:     instanceName,
				Admin:            false,
				MarsocVersion:    martianVersion,
	...
	app.Get("/api/get-metasamples", func() string {
		...
		return util.MakeJSON(lena.GetMetasamples())
	})
	...
	app.Get("/razor", func() string {
		return util.Render("web/marsoc/templates", "sge.html",
			&MainPage{
				InstanceName:     instanceName,
				Admin:            false,
				MarsocVersion:    martianVersion,
	...
	app.Get("/redstone", func() string {
		return util.Render("web/marsoc/templates", "redstone.html",
			&MainPage{
				InstanceName:     instanceName,
				Admin:            false,
				MarsocVersion:    martianVersion,
