...
Options:
	...
	util.Println("MARSOC - %s (built with Go %s)\n", martianVersion, runtime.Version())
	...
	util.LogTee(path.Join(env["MARSOC_LOG_PATH"], time.Now().Format("20060102150405")+".log"))

	// Do not log the value of these environment variables.
	envPrivate := util.EnvRequire([][]string{
	...
	util.LogInfo("options", "Storage high water mark: %d bytes", maxStorageBytes)
