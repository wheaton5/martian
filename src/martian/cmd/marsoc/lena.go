...
func (self *Lena) LoadDatabase() {
	...
	if err != nil {
		util.LogError(err, "lenaapi", "Could not read database file %s.", self.dbPath)
		return
	}
	...
	if err != nil {
		...
		util.LogError(err, "lenaapi", "Could not parse JSON in %s.", self.dbPath)
	}
}
...
func (self *Lena) ingestDatabase(data []byte) error {
	...
	util.LogInfo("lenaapi", "%d samples, %d bags loaded from %s.", len(samples), len(sbagTable), self.dbPath)
	return nil
}
...
func (self *Lena) GoDownloadLoop() {
	go func() {
		for {
			//util.LogInfo("lenaapi", "Starting download...")
			data, err := self.lenaAPI()
			if err != nil {
				util.LogError(err, "lenaapi", "Download error.")
			} else {
				//util.LogInfo("lenaapi", "Download complete. %s.", humanize.Bytes(uint64(len(data))))
				err := self.ingestDatabase(data)
				if err == nil {
					// If JSON parsed properly, save it.
					ioutil.WriteFile(self.dbPath, data, 0644)
					//util.LogInfo("lenaapi", "Database ingested and saved to %s.", self.dbPath)
				} else {
					util.LogError(err, "lenaapi", "Could not parse JSON from downloaded data: %s", string(data))
				}
			}
