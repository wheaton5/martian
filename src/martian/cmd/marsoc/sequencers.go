...
func (self *Sequencer) getFolderInfo(fname string, runchan chan *Run) (int, error) {
	...
	go func(run *Run) {
		...
		if !startTime.IsZero() {
			run.StartTime = startTime.Format(util.TIMEFMT)
		}
		if !completeTime.IsZero() {
			run.CompleteTime = completeTime.Format(util.TIMEFMT)
		}
		if !touchTime.IsZero() {
			run.TouchTime = touchTime.Format(util.TIMEFMT)
		}
...
func (self *SequencerPool) LoadCache() {
	...
	if err != nil {
		util.LogError(err, "seqpool", "Could not read cache file %s.", self.cachePath)
		return
	}
	if err := json.Unmarshal(bytes, &self.folderCache); err != nil {
		util.LogError(err, "seqpool", "Could not parse JSON in cache file %s.", self.cachePath)
		return
	}

	self.indexCache()
	util.LogInfo("seqpool", "%d runs loaded from cache.", len(self.runList))
}
...
func (self *SequencerPool) inventorySequencers() {
	...
	if len(self.runList) > oldRunCount {
		util.LogInfo("seqpool", "%d new runs written to cache. %d total.", len(self.runList)-oldRunCount, len(self.runList))
	}
}
