...
func NewDatabaseManager(name string, url string) *DatabaseManager {
	...
	if err := self.open(); err != nil {
		util.LogInfo("keplerd", "Unable to establish connection with database %s: %s", self.url, err.Error())
		os.Exit(1)
	}
...
func (self *DatabaseManager) initDriver() {
	...
	case "sqlite3":
		break
	default:
		util.LogInfo("keplerd", "Invalid database driver: %s. Valid database drivers: %s", self.name, strings.Join(validNames, ", "))
		os.Exit(1)
	}
}
...
func (self *DatabaseManager) GetProgram(programName string, cycleId int) (*Program, error) {
	...
	if len(res.Rows) == 0 {
		return nil, &util.MartianError{fmt.Sprintf("Failed to find battery for program %s", programName)}
	}
	...
	if len(res.Rows) == 0 {
		return nil, &util.MartianError{fmt.Sprintf("Failed to find cycle %d for program %s", cycleId, programName)}
	}
...
func (self *DatabaseManager) GetRound(programName string, cycleId int, roundId int) (*Round, error) {
	...
	if len(res.Rows) == 0 {
		return nil, &util.MartianError{fmt.Sprintf("Failed to find round %d with program %s, cycle %d",
...
func (self *DatabaseManager) GetTest(testName string) (*Test, error) {
	...
	if len(res.Rows) == 0 {
		return nil, &util.MartianError{fmt.Sprintf("Failed to find test %s", testName)}
	}
