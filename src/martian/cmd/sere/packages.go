...
func (self *PackageManager) BuildPackage(name string, target string) error {
	...
	if _, ok := self.building[pid]; ok {
		self.mutex.Unlock()
		return &util.MartianError{fmt.Sprintf("Package %s-%s already being built.", name, target)}
	}
	self.building[pid] = true
	self.mutex.Unlock()

	sakeDir := path.Join(self.packagePath, name, target)
	sakePath := path.Join(sakeDir, util.GetFilenameWithSuffix(sakeDir, packageBase))
	os.RemoveAll(sakePath)
	os.MkdirAll(sakePath, 0777)

	util.LogInfo("package", "Package %s-%s building in %s.", name, target, sakePath)
	cmd.Dir = sakePath

	go func() {
		if output, err := cmd.CombinedOutput(); err != nil {
			util.LogError(err, "package", "Package %s-%s failed to build in %s: %s",
				name, target, sakePath, string(output))
			os.RemoveAll(sakePath)
		} else {
			util.LogInfo("package", "Package %s-%s successfully built in %s.", name, target, sakePath)
			if err := self.addPackage(name, target, sakePath); err != nil {
				util.LogError(err, "package", "Package %s-%s in %s cannot be added.",
...
func (self *PackageManager) GetPackage(name string, target string, mroVersion string) (*manager.Package, error) {
	...
	return nil, &util.MartianError{fmt.Sprintf("Package %s-%s with mro version %s not found.", name, target, mroVersion)}
}
...
func (self *PackageManager) verifyPackages() {
	...
	if _, ok := util.SearchPaths("sake", paths); !ok {
		util.PrintInfo("package", "Cannot find sake along PATH.")
		os.Exit(1)
	}
		...
		for _, targetInfo := range targetInfos {
			...
			for {
				...
				if err := self.addPackage(name, target, sakePath); err != nil {
					...
					util.LogInfo("package", "Package %s-%s in %s added.",
						name, target, sakePath)
				}
