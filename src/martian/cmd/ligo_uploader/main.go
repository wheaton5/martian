...
func LookupCallInfo(basepath string) (string, string, string) {
	...
	_, _, ast, err := syntax.Compile(basepath+"/_mrosource", []string{}, false)
	if err != nil {
	...
	sampleid := syntax.SearchPipestanceParams(ast, "sample_id")
	if sampleid == nil {
		log.Printf("Could not find a sample_id parameter. Giving up.")
		os.Exit(1)
	}

	desc := syntax.SearchPipestanceParams(ast, "sample_desc")
	if desc == nil {
		desc = ""
	}
