...
function main() {
  ...
  google.charts.setOnLoadCallback(function() {
    ...
    global_table = new google.visualization.Table(
      document.getElementById("table1")
    );
    global_vandv = new google.visualization.Table(
      document.getElementById("table2")
    );
    global_details = new google.visualization.Table(
      document.getElementById("details1")
    );
    global_compare = new google.visualization.Table(
      document.getElementById("compare1")
    );
    global_metrics_table = new google.visualization.Table(
      document.getElementById("list1")
    );
    google.visualization.events.addListener(
      global_metrics_table,
      "select",
      global_view_state.metrics_list_click
    );
  ...
  switch (mode) {
    case "compare":
      ...
      if (!g.compareidnew || !g.compareidold) {
        set_error_box(
          "Please select two rows to compare. Then click compare again."
        );
        if (g.compareidold) {
    ...
    case "details":
      ...
      if (g.compareidold) {
        ...
        set_error_box(
          "Please select one row to view. Then click details again."
        );
        g.mode = "table";
      }
      break;
...
ViewState.prototype.render = function() {
  ...
  switch (w) {
    ...
    case "plot":
      ...
      get_json_safe("/api/list_metrics?metrics_def=" + this.project, function(
        data
      ) {
...
ViewState.prototype.compare_update = function() {
  ...
  if (this.comparemode == "all") {
    url =
      ...
      "&where=" +
      encodeURIComponent(
        "StageName NOT IN ('REPORT_COVERAGE','REPORT_LENGTH_MASS','_perf')"
      );
  } else {
    url =
      "/api/compare?base=" +
...
ViewState.prototype.details_update = function() {
  ...
  get_json_safe(url, function(data) {
    ...
    var options = {
      allowHtml: true,
      cssClassNames: { tableCell: "chart-cell" }
    };
...
ViewState.prototype.table_update = function() {
  ...
  if (mode == "metrics") {
    ...
    var url =
      "/api/plot?where=" +
      where +
      "&columns=test_reports.id,SHA,userid,finishdate,sampleid,testgroup,comments";
  }
...
ViewState.prototype.compute_where_param = function() {
  var clauses = [
    this.where,
    whereify(this.where_sampleid, "sampleid"),
    whereify(this.where_user, "userid")
  ];
  var res = "";
...
function update_model_from_ui() {
  ...
  if (v.mode == "table") {
    if (selected[0]) {
      v.sample_search = get_data_at_row(
        global_table_data,
        "sampleid",
        selected[0].row
      );
      v.compareidold = get_data_at_row(
        global_table_data,
        "test_reports.id",
        selected[0].row
      );
    ...
    if (selected[1]) {
      v.compareidnew = get_data_at_row(
        global_table_data,
        "test_reports.id",
        selected[1].row
      );
